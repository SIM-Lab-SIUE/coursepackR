...
```{r clean-data}
# We use mutate() and across() to change all 5 columns at once
# We will create 5 new variables with an "_r" suffix
w144_clean <- w144_excerpt %>%
  mutate(
    across(
      .cols = starts_with("fb_news_"),
      .fns = ~ recode(., `1` = 4, `2` = 3, `3` = 2, `4` = 1),
      .names = "{.col}_r"  # This creates the new col names
    )
  )

# Let's check our work
w144_clean %>%
  select(fb_news_friends, fb_news_friends_r) %>%
  head()
```
