...
logistic_lm <- glm(us_favorable ~ age + ideology_numeric, 
                   data = pew_clean, 
                   family = binomial)
...
full_model <- lm(conf_biden ~ age + age_squared + ideology_numeric + gender_numeric, 
                 data = pew_clean)
...
set.seed(123)
train_indices <- sample(1:nrow(pew_clean), 0.7 * nrow(pew_clean))
train_data <- pew_clean[train_indices, ]
test_data <- pew_clean[-train_indices, ]

# Fit on training data
cv_model <- lm(conf_biden ~ age + ideology_numeric, data = train_data)

# Predict on test data
predictions <- predict(cv_model, newdata = test_data)

# Calculate RMSE
rmse <- sqrt(mean((test_data$conf_biden - predictions)^2, na.rm = TRUE))
rmse
```
...
left_data <- filter(pew_clean, ideology_simple == "Left")
right_data <- filter(pew_clean, ideology_simple == "Right")

d <- cohen_d(mean(left_data$conf_biden, na.rm = TRUE),
             mean(right_data$conf_biden, na.rm = TRUE),
             sd(left_data$conf_biden, na.rm = TRUE),
             sd(right_data$conf_biden, na.rm = TRUE),
...
anova_result <- aov(conf_biden ~ ideology_simple, data = pew_clean)
...
bootstrap_differences <- replicate(1000, {
  boot_sample <- pew_clean %>% sample_n(nrow(pew_clean), replace = TRUE)
  left_boot <- filter(boot_sample, ideology_simple == "Left")
  right_boot <- filter(boot_sample, ideology_simple == "Right")
  mean(left_boot$conf_biden, na.rm = TRUE) - mean(right_boot$conf_biden, na.rm = TRUE)
})
