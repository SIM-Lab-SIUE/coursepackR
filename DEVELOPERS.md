...
This R package provides course materials and environment setup automation for MC 451 and MC 501 at SIUE. It's designed to minimize student setup friction by automating software installation and providing easy access to weekly assignment templates.
...
|----------|---------|-------------|
| `mccourse_setup()` | Install Quarto, TinyTeX, core packages | Run once after package install |
| `mccourse_self_test()` | Verify environment setup | Diagnose student issues |
| `list_courses()` | Show available courses | Discovery |
| `list_weeks()` | Show available weeks | Discovery |
| `download_week()` | Copy week template locally | Get weekly materials |
| `download_journal()` | Copy journal template | Get journal scaffold |
| `open_week()` | Open folder in file browser | Quick navigation |
| `mccourse_update()` | Update package to latest | Stay current |
| `final_project()` | Create project structure | Final submissions |

### Internal Functions (Not Exported)

All functions in `utils-paths.R` and check functions in `check_env.R` are internal helpers marked with `@keywords internal`.
...
4. Test: `list_courses()` should show new course
...
Then use:
- `newpackage::function()` for explicit calls (preferred)
...
Check:
1. Course folder exists: `inst/courses/<course>/`
2. Weeks folder exists: `inst/courses/<course>/weeks/`
3. Week folders match pattern: `week_01`, `week_02`, etc.
4. Package was rebuilt after adding materials

#### Students can't install package

Check:
1. All dependencies in DESCRIPTION
2. No syntax errors: `devtools::check()`
3. GitHub repo is public
4. Students using correct install command
...
- [ ] Support for custom template locations
- [ ] Better handling of conflicts/overwrites
