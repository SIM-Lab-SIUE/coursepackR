...
-   The first block takes the data AND THEN `arrange`s the rows by 'F_AGE' (from youngest to oldest).
...
-   **Step 2:** Then, `select` only the 'party' and 'F_AGE' columns.
-   **Step 3:** Finally, `arrange` the result by 'F_AGE' in descending order.
...
-   **Step 4a: Recode 'fb_why' items to 1/0** (Inside the first `mutate()`)
