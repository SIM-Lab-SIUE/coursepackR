...
### Step 1.3: Calculate the Percentage of D6 in Your Sample

To calculate the percentage of D6 dice in your sample, use the following code. This will divide the count of D6 by the total number of dice in your sample and multiply by 100 to get the percentage.

```{r}
#| label: calculate-d6-percentage
# Calculate the percentage of D6 dice in your sample
d6_percentage <- my_dice_sample %>%
  filter(die_type == "D6") %>%
  mutate(percentage = (count / sum(my_dice_sample$count)) * 100)
...
- `filter(die_type == "D6")`: Selects only the row where the die type is D6.
- `mutate(percentage = ...)`: Adds a new column to calculate the percentage of D6 dice in your sample.
- `sum(my_dice_sample$count)`: Calculates the total number of dice in your sample.
...
- What percentage of your sample is D6? Refer to the result from the `d6_percentage` calculation above.
- How does this compare to the true proportion of 28.8%?
