...
**Lab 14: Inferential Statistics** **Due Date:** Week 14, end of lab period
...
```{r}
# Create a table of the two categorical variables
...
```{r}
# Run the test on the table object you just created
...
-   **HA (Alternative):** The mean `platform_count` for men is **not the same as** the mean `platform_count` for women. (Mean 1 ≠ Mean 2)
...
```{r}
# 1. Create a dataset that removes anyone who didn't answer 'gender'
# 'is.na()' means "is missing" and '!' means "NOT"
gender_data <- w144_wrangled %>% 
  filter(!is.na(gender))

# 2. Run the t.test on this new, clean data
...
```{r}
# 2. Look at the summary of the model
summary(anova_model)
```
...
To get credit for this lab, save your `Lab_14.R` script.

If you have extra time, try this **challenge**:

1.  **Run a new, full analysis** (from `aov()` to `TukeyHSD()`) to answer this question: "Is there a statistically significant difference in `fb_uses_count` (the scale you built in Lab 12) across our three `party_simple` groups?"
2.  **Write a comment (`#`)** above your code explaining your final conclusion in plain English. (Did you reject the null? What does the Tukey test show?)