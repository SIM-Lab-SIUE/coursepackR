test_that("download_week creates a week_XX folder and copies files when present", {
  ...
  for (course in courses) {
    weeks <- list_weeks(course)  # like "week_01", "week_02", ...
    ...
    for (wk in weeks) {
      ...
      # destination must be td/week_XX
      dest_dir <- file.path(td, wk)
      expect_true(dir.exists(dest_dir))

      # source files (exclude dirs)
      ...
      # destination files (exclude dirs)
      dest_files <- list.files(dest_dir, recursive = TRUE, all.files = TRUE, full.names = TRUE, no.. = TRUE)
      dest_files <- dest_files[!dir.exists(dest_files)]

      if (length(src_files) == 0L) {
        expect_equal(length(dest_files), 0L)  # empty week still makes folder
      } else {
