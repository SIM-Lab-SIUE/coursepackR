...
In Lab 12, you did the hard work of "wrangling"—you created new, meaningful variables like `party_simple` and `platform_count`. But data wrangling is just the prep work. This week, we learn how to **explore** that data.
...
```
  ...
  * **Line 1:** Get a summary for your 'platform\_count' scale.
  * **Line 3:** Get a summary for your 'fb\_uses\_count' scale.
...
```
  ...
  * **Lines 1-2:** Count the 'party\_simple' variable you created.
  * **Lines 4-5:** You can also sort the results. `'sort = TRUE'` lists the biggest group first.
...
**Question:** How many people are in each `party_simple` group?
**Plot:** A bar chart.
...
```
  ...
  * **1. Setup:** `data` is 'w144\_wrangled', `x-axis` is 'party\_simple'.
...
**Question:** What is the distribution of our `platform_count` scale?
**Plot:** A histogram.
...
```
  ...
  * **1. Setup:** `data` is 'w144\_wrangled', `x-axis` is 'platform\_count'.
...
**Question:** Does `platform_count` (numeric) differ by `age_group` (categorical)?
**Plot:** A violin plot (or boxplot). A violin plot is like a histogram for each group, turned on its side.
...
```
  ...
  * **1. Setup:** `x-axis` is 'age\_group', `y-axis` is 'platform\_count'.
...
**Question:** What is the *proportion* of TikTok use (`uses_tiktok`) in each `age_group`?
**Plot:** A 100% "filled" bar chart.
...
```
  ...
  * **1. Setup:** `x-axis` is 'age\_group', 'fill' color is 'uses\_tiktok'.
...
```
  ...
  * **1. Setup:** A basic histogram of 'fb\_uses\_count'.
