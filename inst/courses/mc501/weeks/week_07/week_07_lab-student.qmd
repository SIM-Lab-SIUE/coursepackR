...
When you load the `mccoursepack` package, the excerpted Pew dataset is available as `ATP_W144_excerpt`. Load it just like you would with other famous datasets (e.g., `penguins` from `palmerpenguins`).
...
#### Example Summaries with the New Dataset

What is the average age category (`F_AGECAT`) in your sample?

```{r}
mean_agecat <- mean(my_sample$F_AGECAT, na.rm = TRUE)
mean_agecat
```

Make a bar chart of the `F_AGECAT` distribution in your sample:

```{r}
my_sample %>%
  count(F_AGECAT) %>%
  ggplot(aes(x = as.factor(F_AGECAT), y = n, fill = as.factor(F_AGECAT))) +
  geom_col() +
  theme_minimal() +
  labs(
    title = "Age Category Distribution in My Sample",
    x = "F_AGECAT",
    y = "Count"
  )
```
...
-   How does the average `F_AGECAT` in your sample compare to the whole dataset? Try calculating the mean for the full dataset and compare.
-   How does the distribution of `F_AGECAT` in your sample compare to the full dataset?
