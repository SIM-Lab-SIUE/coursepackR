...
# Worksheet 14: Inferential Statistics (Modernized)

## Learning Objectives

This worksheet will help you practice the concepts from Lab 14:
...
You will use the same dataset as in Lab 14. Make sure you have run your "wrangle chunk" to create the `pew_wrangled` object.
...
Write a null hypothesis (H0) and an alternative hypothesis (HA) for the following research question:

**Research Question:** Are there differences in TikTok use by party identification?
...
Use the `chisq.test()` function to test the relationship between party identification and TikTok use. Use the `party_simple` and `uses_tiktok` variables in your `pew_wrangled` dataset.
...
Use the `t.test()` function to compare the mean number of platforms used (`platform_count`) between two income groups. Use the `income` and `platform_count` variables in your `pew_wrangled` dataset. (Hint: You may need to filter out missing or refusal values.)

```{r}
reg_data <- pew_wrangled %>% filter(!is.na(income), income != 99, !is.na(platform_count))
t.test(platform_count ~ income, data = reg_data)
```
## 4. ANOVA

Use the `aov()` function to test for differences in `platform_count` by age group. Use the `age_group` and `platform_count` variables in your `pew_wrangled` dataset.
...
Use the `lm()` function to predict `platform_count` from `income`, `gender`, and `party_simple` in your `pew_wrangled` dataset. (Hint: Remove missing values.)
